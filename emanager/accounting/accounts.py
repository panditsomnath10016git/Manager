...
class Account(Transaction):
    def __init__(self, acc_no):
        ...
        self.acc_no = acc_no
        self.acc_details = pd.read_csv(f"{acc_data_path}/chart_of_accounts.csv", index_col='ACCOUNT_NO').loc[acc_no]
...
class CreateAccount:
    ...
    def __init__(self, name, address, mobile_no, first_deposit=0.0):
        ...
        if not acc_chart.isin([name, mobile_no]).any().any():
            ...
            acc_data.to_csv(f"{acc_data_path}/chart_of_accounts.csv", mode="a", header=False, index=False)
    ...
    #TODO make acc_no mapped with ID no
        print('generating account number...')
        last_acc_no = pd.read_csv(f"{acc_data_path}/chart_of_accounts.csv")["ACCOUNT_NO"].max()
